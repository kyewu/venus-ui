<template>
  <Swiper :items="homeStore.swiperLists" :height="(store.rate * 36) + 'rem'"></Swiper>
  <!-- title -->
  <Container>
    <!-- 标题 -->
    <div class="p-4">
  ...
  <Container class="p-4">
    <div class="grid sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 w-full lt-sm:px-4">
      <a class="flex group transition-all shadow-lg hover:(bg-sky-500 shadow-lg text-white transform-translate-y--1)"
        :href="card.url" target="_blank" v-for="(card, index) in homeStore.projects" :key="index">
  ...
  <Container class="p-4">
    <div class="grid sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 w-full lt-sm:px-4">
      <a class="['group','hover:( shadow-2xl transform-all transform-translate-y--5)']" :href="card.url" target="_blank"
        v-for="(card, index) in homeStore.courses" :key="index">
        <Card v-bind="card" class="w-full rounded-3 shadow-lg transition-all hover:(transform-translate-y--1 shadow-lg)"
          :image-type="'rounded'">
          <template #default>
            <div class="flex flex-row items-center justify-between text-gray-500 mb-4 mx-4">
              <span>查看更多</span>
  ...
  <Container class="w-full text-gray-400 p-4">
    <Swiper class="w-2/3" :items="homeStore['swiper-projects']" :height="(store.rate * 28) + 'rem'"
...
onBeforeMount(async () => {
  ...
  console.log('swipers:', homeStore.swiperLists)
